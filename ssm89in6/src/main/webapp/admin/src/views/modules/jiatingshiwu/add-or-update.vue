<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="事务名称" prop="shiwumingcheng">
          <el-input v-model="ruleForm.shiwumingcheng" 
              placeholder="事务名称" clearable  :readonly="ro.shiwumingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="事务名称" prop="shiwumingcheng">
              <el-input v-model="ruleForm.shiwumingcheng" 
                placeholder="事务名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="事务类型" prop="shiwuleixing">
          <el-select v-model="ruleForm.shiwuleixing" placeholder="请选择事务类型">
            <el-option
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="事务进度" prop="shiwujindu">
          <el-select v-model="ruleForm.shiwujindu" placeholder="请选择事务进度">
            <el-option
                v-for="(item,index) in shiwujinduOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="事务进度" prop="shiwujindu">
	      <el-input v-model="ruleForm.shiwujindu"
                placeholder="事务进度" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="事务详情" prop="shiwuxiangqing">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="事务详情"
                  v-model="ruleForm.shiwuxiangqing" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shiwuxiangqing" label="事务详情" prop="shiwuxiangqing">
                    <span>{{ruleForm.shiwuxiangqing}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/jiatingchengyuan/zhanghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
    ...
    zhanghaoChange () {
      this.$http({
        url: `follow/jiatingchengyuan/zhanghao?columnValue=`+ this.ruleForm.zhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jiatingshiwu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiatingshiwuCrossAddOrUpdateFlag = false;
                  this.parent.search();
